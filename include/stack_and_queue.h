...
public:
    ...
    Vector(size_t size) : sz(size), capacity(size)
    {
        if (capacity< 0) throw ("capacity should be greater than zero");
        if (capacity == 0) capacity = 10;
        pMem = new T[sz]();
        if (pMem == nullptr) throw"pMem is nullptr";
    }

    Vector(T* arr, size_t s) : sz(s), capacity(s)
    {
